...
int main(void)
{
    ...
    if (sdrplay_api_Open() != sdrplay_api_Success ||
        ...
        count == 0u || sdrplay_api_SelectDevice(&devices[0]) != sdrplay_api_Success ||
        sdrplay_api_GetDeviceParams(devices[0].dev, &params) != sdrplay_api_Success) return EXIT_FAILURE;

    params->devParams->fsFreq.fsHz = 10000000.0;
    params->devParams->mode = sdrplay_api_BULK;
    params->rxChannelA->tunerParams.rfFreq.rfHz = 853912500.0;
    params->rxChannelA->tunerParams.bwType = sdrplay_api_BW_8_000;
    params->rxChannelA->tunerParams.ifType = sdrplay_api_IF_Zero;
    params->rxChannelA->tunerParams.gain.gRdB = 55;
    params->rxChannelA->tunerParams.gain.LNAstate = 0;
    ...
    for (unsigned int state = 0; state < 10u; ++state) {
        ...
        params->rxChannelA->tunerParams.gain.gRdB = 55;
        params->rxChannelA->tunerParams.gain.LNAstate = (unsigned char)state;
        sdrplay_api_ErrT result = sdrplay_api_Update(devices[0].dev, sdrplay_api_Tuner_A,
        ...
        printf("GAIN_SWEEP lna=%u gr=55 result=%d rms_squared=%.1f peak=%u current=%.1f\n",
               state, result, mean_square, atomic_load(&peak_sample),
    ...
    for (unsigned int lna_index = 0; lna_index < 2u; ++lna_index) {
        for (unsigned int gr_index = 0; gr_index < 5u; ++gr_index) {
            params->rxChannelA->tunerParams.gain.gRdB = reductions[gr_index];
            params->rxChannelA->tunerParams.gain.LNAstate = (unsigned char)lna_states[lna_index];
            sdrplay_api_ErrT result = sdrplay_api_Update(devices[0].dev, sdrplay_api_Tuner_A,
                                                         sdrplay_api_Update_Tuner_Gr, 0u);
    ...
    params->rxChannelA->tunerParams.gain.gRdB = 20;
    params->rxChannelA->tunerParams.gain.LNAstate = 5;
    (void)sdrplay_api_Update(devices[0].dev, sdrplay_api_Tuner_A,
                             sdrplay_api_Update_Tuner_Gr, 0u);
    params->rxChannelA->ctrlParams.agc.setPoint_dBfs = -30;
    params->rxChannelA->ctrlParams.agc.enable = sdrplay_api_AGC_50HZ;
    sdrplay_api_ErrT agc_result = sdrplay_api_Update(devices[0].dev, sdrplay_api_Tuner_A,
                                                     sdrplay_api_Update_Ctrl_Agc, 0u);
    pause_ms(2000);
    int converged_reduction = params->rxChannelA->tunerParams.gain.gRdB;
    printf("AGC_TEST enabled_result=%d start_gr=20 converged_gr=%d current=%.1f\n",
           agc_result, converged_reduction,
           params->rxChannelA->tunerParams.gain.gainVals.curr);
    params->rxChannelA->tunerParams.gain.gRdB = 59;
    (void)sdrplay_api_Update(devices[0].dev, sdrplay_api_Tuner_A,
                             sdrplay_api_Update_Tuner_Gr, 0u);
    pause_ms(2000);
    printf("AGC_TEST recovery_start_gr=59 recovered_gr=%d current=%.1f\n",
           params->rxChannelA->tunerParams.gain.gRdB,
           params->rxChannelA->tunerParams.gain.gainVals.curr);
    converged_reduction = params->rxChannelA->tunerParams.gain.gRdB;
    params->rxChannelA->ctrlParams.agc.enable = sdrplay_api_AGC_DISABLE;
    sdrplay_api_ErrT disable_result = sdrplay_api_Update(devices[0].dev, sdrplay_api_Tuner_A,
                                                         sdrplay_api_Update_Ctrl_Agc, 0u);
    pause_ms(500);
    printf("AGC_TEST disabled_result=%d held_gr=%d expected_gr=%d\n", disable_result,
           params->rxChannelA->tunerParams.gain.gRdB, converged_reduction);
