...
MIRISDR_API int mirisdr_open_tuner (mirisdr_dev_t **p, uint32_t index,
                                     unsigned int tuner);
...
MIRISDR_API uint32_t mirisdr_rspduo_format_word(uint32_t rate);
MIRISDR_API uint32_t mirisdr_rspduo_format_samples(uint32_t rate);
...
#define MIRISDR_RSPDUO_CHANGE_AM_PORT (1u << 4)
#define MIRISDR_RSPDUO_CHANGE_AM_NOTCH (1u << 5)
